...
2. **Raw uploaded documents are not persisted to storage.** Images/PDFs are
...
3. **Per-claim limit (₹5,000) only applies to a claim's gross amount when
...
4. **Copay and network discount are computed on the claim total, not
...
5. **MRI/CT scans always require pre-authorization**, regardless of claim
...
6. **Missing `member_join_date` defaults to the policy's `effective_date`
...
7. **Doctor registration numbers accept both the documented 3-segment
...
8. **`SERVICE_NOT_COVERED` vs. `EXCLUDED_CONDITION`**: treatments that
...
9. **No real employee/member database exists — `data/member_roster.json`
...
10. **`PATIENT_MISMATCH` now runs on a real extracted field.** The LLM
...
11. **`ILLEGIBLE_DOCUMENTS` is now driven by the LLM's own extraction
...
12. **`ANNUAL_LIMIT_EXCEEDED` is now computed from real approved-claim
...
13. **`DUPLICATE_CLAIM` is now computed from real claim history**: an exact
...
14. **`LATE_SUBMISSION` is deliberately NOT wired to the real submission
...
15. **`DATE_MISMATCH` remains untested via the UI.** It would need
...
16. **Fraud detection remains a simplified heuristic set**, not a trained
...
17. **`previous_claims_same_day` and every other now-real signal above are
    computed server-side**, never user-submitted — a fraudulent or careless
    claimant wouldn't self-report any of these accurately.

## AI behavior

18. **Confidence scores are derived from rule-check tiers** (how mechanical
...
19. **One appeal per claim, no multi-round appeal chains.** A claim that's
    ...
    for.
20. **Only `REJECTED` and `PARTIAL` claims are appealable.** `APPROVED` has
...
21. **An appeal's `overrideAmount` can never exceed the claim's originally
...
22. **Overturning an appeal changes `Claim.status` but never touches the
...
23. **Policy configuration is a single global row, not versioned or
...
24. **The admin-editable policy falls back to the static
...
25. **A second, separate admin credential (`ADMIN_PASSWORD`), not the same
    ...
    Limitations.
26. **AI accuracy metrics use fixed confidence-bucket thresholds (≥0.90
    ...
    existing `ILLEGIBLE_DOCUMENTS` cutoff (assumption #11 above) so the
